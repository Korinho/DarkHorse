...
class ControllerCandidato{
	...
	if(isset($_POST["first_name"])){
			...
			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["first_name"]) &&
			   ...
			   preg_match('/^[a-zA-Z0-9]+$/', $_POST["pass"])){
				...
				$datos = array("nombre"=>$_POST["first_name"],
				...
				$respuesta = ModelCandidato::RegistroCandidato($tabla, $datos);
				...
				if($respuesta == "ok"){
					...
					$mail->setFrom('webmaster@darkhorserecruitment.com', 'darkhorserecruitment');

					$mail->addReplyTo('webmaster@darkhorserecruitment.com', 'darkhorserecruitment');
					...
					if(!$envio){
						...
						echo '<script> 
							...
							swal({
								  title: "¡OK!",
								  text: "¡Por favor revise la bandeja de entrada o la carpeta de SPAM de su correo electrónico '.$_POST["email"].' para verificar la cuenta!",
								  type:"success",
								  confirmButtonText: "Cerrar",
								  closeOnConfirm: false
								},
	...
	static public function ctrMostrarCandidatoINT($item,$valor){

		$tabla = "candidatos";

		$respuesta = ModelCandidato::MostrarCandidatoINT($tabla, $item, $valor);

		return $respuesta;

	}
	static public function ctrMostrarCandidatoSTR($item,$valor){

		$tabla = "candidatos";

		$respuesta = ModelCandidato::MostrarCandidatoSTR($tabla, $item, $valor);

		return $respuesta;

	}

	static public function ctrContarVacantes($id){

		$tabla = "vacante_candidato";

		$respuesta = ModelCandidato::mdlContarVacantes($tabla,$id);
	...
	static public function ctrActualizarUsuario($id, $item, $valor){
		...
		$respuesta = ModeloUsuarios::ActualizarCandidato($tabla, $id, $item, $valor);
	...
	public function ctrIngresoCandidato(){

		if(isset($_POST["usuario"])){

			if(preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["usuario"]) &&
			   preg_match('/^[a-zA-Z0-9]+$/', $_POST["pass"])){
				...
				$respuesta = ModeloUsuarios::MostrarCandidato($tabla, $item, $valor);

				if($respuesta["email"] == $_POST["usuario"] && $respuesta["password"] == $encriptar){
	...
	public function ctrOlvidoPassword(){
		...
		if(isset($_POST["passEmail"])){
			...
			if(preg_match('/^[^0-9][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[@][a-zA-Z0-9_]+([.][a-zA-Z0-9_]+)*[.][a-zA-Z]{2,4}$/', $_POST["passEmail"])){
				...
				$respuesta1 = ModelCandidato::MostrarCandidato($tabla, $item1, $valor1);
				...
				if($respuesta1){
					...
					$respuesta2 = ModelCandidato::ActualizarCandidato($tabla, $id, $item2, $valor2);
	...
	static public function ctrRegistroRedesSociales($datos){
		...
		$respuesta0 = ModeloUsuarios::MostrarCandidato($tabla, $item, $valor);
		...
		if($respuesta0){
			...
			$respuesta1 = ModelCandidato::RegistroCandidato($tabla, $datos);
		...
		if($emailRepetido || $respuesta1 == "ok"){
			...
			$respuesta2 = ModeloUsuarios::MostrarCandidato($tabla, $item, $valor);
	...
	public function ctrActualizarPerfil(){
		...
		if(isset($_POST["editarNombre"])){
			...
			$respuesta = ModeloCandidato::ActualizarPerfilCandidato($tabla, $datos);
	...
	static public function ctrMostrarCompras($item, $valor){
		...
		$respuesta = ModelCandidato::mdlMostrarCompras($tabla, $item, $valor);
	...
	/*=============================================
	MOSTRAR LISTA DE DESEOS - mostrar postulaciones
	=============================================*/
	...
	static public function ctrMostrarCandidatos(){
		...
		$respuesta = ModelCandidato::mdlMostrarCandidatos($tabla);
	...
	/*=============================================
	QUITAR PRODUCTO DE LISTA DE DESEOS - eliminar postulacion
	=============================================*/
